...
class StatsArrays:
    ...
    def __init__(self, array_files=array_files_default, end_files=array_files_end_default,
        ...
        self.array_files = array_files if isinstance(array_files, list) else [array_files]
        self.end_files = end_files if isinstance(end_files, list) else [end_files]
        ...
        files = [file + str(self.iter_start) + end + self.ext for file in self.array_files for end in self.end_files]
    ...
    def mean(self):
        ...
        for array_file in self.array_files:
            for end_file in self.end_files:
                cpt = 0
                file_name = os.path.join(self.in_dir, array_file + str(self.iter_start) + end_file + self.ext)
                #print(file_name)
                ...
                if file_exists:
                    ...
                    while(file_exists):
                        ...
                        file_name = os.path.join(self.in_dir, array_file  + str(self.iter_start + cpt) + end_file + self.ext)
                        (data, file_exists) = self.read_file(file_name)
        ...
        for (i, array_file) in enumerate(self.array_files):
            for (j, end_file) in enumerate(self.end_files):
                ...
                if file_exists:
                    ...
                    while(file_exists):
                        mean = self.means[array_file + end_file + "_moy" + self.ext]
                        if self.ignore_col is not None:
                    ...
                    std /= cpt;
                    stds[array_file + end_file + "_et" + self.ext] = np.sqrt(std)
    ...
    def write_results(self, out_dir="Results", file_out="results_stats_obs.txt"):
        ...
        files_moy = [file  + end + "_moy" + self.ext for file in self.array_files for end in self.end_files]
        files_et = [file  + end + "_et" + self.ext for file in self.array_files for end in self.end_files]
